...
| A selected player character cannot silently omit a role or weapon attachment | 0/3 | app resolves all eight role names and the `Weapon` joint before atomic selection; fixture and exhaustive profile-mapping tests cover the catalog |
...
| Stable horde phase variation needs no per-enemy storage or steady-frame allocation | 0/3 | app hashes generational `EntityId` into four phases per role and reuses eight instance vectors; dense-swap and capacity tests pin both properties |
