...
| Animation data cannot exceed the first clip's channel, keyframe, or duration budgets | 0/1 | private `AnimationClip` and `AnimationChannel` fields; import admission at `MAX_ANIMATION_CHANNELS`, `MAX_KEYFRAMES_PER_CHANNEL`, and `MAX_CLIP_SECONDS`; unused accessors and views are rejected |
